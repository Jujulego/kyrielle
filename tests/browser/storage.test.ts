...
import { storage$ } from '@/src/browser/storage.js';
...
describe('storage$', () => {
  it('should set initial value if key misses in storage', () => {
    storage$(localStorage, 'life', '42');

    expect(localStorage.getItem('life')).toBe('42');
  });

  it('should return readable reference on storage item', () => {
    localStorage.setItem('life', '42');

    const ref = storage$(localStorage, 'life');
  ...
  it('should listen to window storage events', () => {
    ...
    // Emit update event
    callback({ storageArea: localStorage, key: 'life', newValue: '42' } as StorageEvent);
